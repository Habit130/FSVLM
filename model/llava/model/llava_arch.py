...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        vision_tower = model_args.vision_tower#模型参数
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        image_features = self.get_model().get_vision_tower()(images.cuda())
    ...
    def _get_past_length(self, past_key_values):
        ...
        for layer_cache in reversed(past_key_values):
            if layer_cache is None:
                continue
            for cache_tensor in reversed(layer_cache):
                if cache_tensor is not None:
                    return cache_tensor.shape[-2]
        return None
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(images.cuda())
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while image_token_indices.numel() > 0:
                # print('image_token_indices.numel()',image_token_indices.numel())
                cur_image_features = image_features[cur_image_idx]
                # print('image_token_indices[0]',image_token_indices[0])
